...
def analyze_materials_with_deepseek(materials: list, past_exams: str = None) -> dict:
    ...
    prompt = f"""Ты — AI-эксперт по подготовке к экзаменам в университете. Проанализируй учебные материалы студента и спрогнозируй вопросы на экзамене.
...
2. Если есть прошлогодние билеты — учти паттерны (какие темы повторяются, стиль вопросов, формулировки)
3. Выдели акценты преподавателя (повторяющиеся темы, подчёркнутое, определения)
...
- Вопросы должны быть реалистичными для университетского экзамена
- Стиль вопросов должен соответствовать материалам и прошлогодним билетам
- Распределение сложности: 5 лёгких (easy), 10 средних (medium), 5 сложных (hard)
- Вопросы покрывают разные темы, но с акцентом на ключевые
- Ответы краткие (3-5 предложений), но содержательные и точные

ПРИМЕРЫ ХОРОШИХ ВОПРОСОВ:
✅ "Объясните принцип работы алгоритма быстрой сортировки и его сложность"
✅ "В чём разница между TCP и UDP протоколами? Когда использовать каждый?"
✅ "Докажите теорему Пифагора геометрическим методом"

ПРИМЕРЫ ПЛОХИХ ВОПРОСОВ:
❌ "Что такое сортировка?" (слишком общо)
❌ "Расскажите всё про сети" (слишком широко)
❌ "Кто изобрёл теорему Пифагора?" (не по существу)
