...
def generate_fact(subject_key):
    ...
    prompt = f"""Придумай один короткий, интересный и удивительный факт по предмету «{subject_name}».
Факт должен быть:
- Полезным для подготовки к ЕГЭ/ОГЭ
- Удивительным — чтобы хотелось поделиться
- Коротким — максимум 2 предложения (до 150 символов)
- Без вступления, просто сам факт

Ответь ТОЛЬКО JSON: {{"text": "...", "emoji": "..."}}
Emoji — один символ, подходящий к факту."""

    try:
        http = httpx.Client(timeout=httpx.Timeout(10.0, connect=3.0))
        resp = http.post(
            API_URL,
            json={
                'model': MODEL,
                'messages': [
                    {'role': 'system', 'content': 'Ты генератор интересных фактов для школьников. Отвечай только JSON.'},
