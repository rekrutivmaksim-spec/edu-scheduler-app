...
def html_welcome(name: str) -> str:
    return _base(
        ...
        _body(
            _hi(name) +
            _text('У тебя уже есть <strong style="color:#6366f1;">3 дня Premium бесплатно</strong>. Задай первый вопрос — и увидишь, как это работает.') +
...
def html_comeback(name: str) -> str:
    return _base(
        ...
        _body(
            _hi(name) +
            _text('Вчера ты зарегистрировался, но ещё не начал заниматься. <strong>Один вопрос — и ты уже впереди</strong> большинства учеников.') +
        ...
        preview='Вчера зарегистрировался — сегодня начни учиться!'
    )
...
def html_trial_ending(name: str) -> str:
    return _base(
        ...
        _body(
            ...
            _text('<span style="color:#9ca3af;font-size:13px;">Первый месяц — всего 499₽.</span>')
        ),
        preview='Завтра лимит снизится до 3 вопросов — продли сейчас'
    )
