...
def _is_ocr_refusal(text: str) -> bool:
    refusal_markers = [
        ...
        'извините', 'не могу помочь', 'i\'m sorry', 'i am sorry',
        'не могу выполнить', 'не могу обработать', 'cannot process',
        'cannot help', 'can\'t help', 'не в состоянии',
    ]
    lower = text.lower()
...
def _ocr_and_solve(image_base64: str, hint: str = '') -> dict:
    ...
    ocr_prompt = (
        "Внимательно рассмотри это фото. "
        "Если на фото есть текст (задача, надпись, формула, условие) — перепиши ВЕСЬ текст дословно, по-русски. "
        "Если текста нет или его мало — подробно опиши что изображено на фото (предметы, обстановка, детали). "
        "Отвечай по-русски, без комментариев, без иероглифов, без markdown-заголовков."
    )
    ...
    if not recognized_text:
        return {'recognized_text': '', 'solution': 'Не удалось распознать фото. Попробуй сфотографировать чётче при хорошем освещении.', 'subject': 'Общее'}
    ...
    if len(recognized_text) < 5:
        return {'recognized_text': '', 'solution': 'Не удалось распознать содержимое фото. Попробуй сфотографировать чётче.', 'subject': 'Общее'}

    hint_text = f"\nДополнительно от пользователя: {hint}" if hint else ""
    solve_prompt = (
        f"Содержимое с фото:{hint_text}\n\n{recognized_text}\n\n"
        "ВАЖНО: Определи что на фото.\n"
        "- Если это учебная задача, уравнение, тест — реши ПОЛНОСТЬЮ пошагово.\n"
        "- Если это НЕ задача (бытовой предмет, кафель, еда, пейзаж, техника, документ и т.д.) — "
        "опиши что это, дай полезную информацию или интересные факты по теме. Никогда не отказывай.\n\n"
        "Верни ответ СТРОГО в JSON-формате (без markdown-обёрток, без ```json):\n"
        '{"steps":[{"icon":"arrow","title":"заголовок шага","text":"подробное объяснение"}],'
        '"answer":"итоговый ответ или описание кратко",'
        '"check":"проверка или дополнительная информация",'
        '"tip":"полезный совет по теме",'
        '"practice":[{"text":"связанный вопрос 1"},{"text":"связанный вопрос 2"}],'
        '"motivation":"короткая дружелюбная фраза"}\n\n'
        "Правила:\n"
        "- steps: 2-5 шагов, icon может быть arrow/pin/check\n"
        "- Формулы текстом: x^2, sqrt(x), a/b\n"
        "- answer: краткий ответ или описание того что на фото\n"
        "- practice: 2 связанных вопроса по теме\n"
    ...
    try:
        resp_s = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": (
                    "Ты умный ассистент Studyfay. "
                    "Если на фото учебная задача — реши ПОЛНОСТЬЮ пошагово, проверяй вычисления. "
                    "Если на фото бытовой предмет, еда, техника, кафель и т.д. — опиши что это и дай полезную информацию. "
                    "Определяй предмет/тему. Формулы текстом. Русский. "
...
def ask_ai(question, context, image_base64=None, exam_meta=None, history=None):
    ...
    md_fmt = "Форматирование: используй **жирный** для ключевых терминов, ## заголовки, нумерованные списки для шагов, > цитаты для правил."
    if exam_meta:
        ...
        system = (
            "Ты Studyfay — умный и дружелюбный ассистент. Помогаешь школьникам, студентам и всем, кто спрашивает.\n\n"
            "КАК ТЫ РАБОТАЕШЬ:\n"
            "• Отвечай на ЛЮБЫЕ вопросы — учебные, бытовые, про предметы, технику, кулинарию, природу и т.д.\n"
            "• Объясняй ПРОСТЫМИ СЛОВАМИ — как будто рассказываешь другу. Используй аналогии из жизни.\n"
            "• Если задача — реши ПОЛНОСТЬЮ пошагово. Каждый шаг на отдельной строке с пояснением «почему».\n"
            "• Если вопрос бытовой или общий — дай полезный, конкретный ответ с примерами.\n"
            "• Если пользователь прислал фото чего-то (кафель, еда, предмет) — опиши что это и помоги.\n"
            "• В КОНЦЕ ответа задай ОДИН короткий вопрос по теме.\n\n"
            "ФОРМАТИРОВАНИЕ (обязательно):\n"
            "• Используй markdown: **жирный** для ключевых терминов и ответов.\n"
            "• Используй ## заголовки для разделов.\n"
            "• Используй нумерованные списки (1. 2. 3.) для пошаговых решений.\n"
            "• Используй маркированные списки (- ...) для перечислений.\n"
            "• Используй > цитаты для важных правил/формул.\n\n"
            "СТИЛЬ:\n"
            ...
            "СТРОГИЕ ЗАПРЕТЫ:\n"
            "• НИКОГДА не отказывай отвечать. Если тема не учебная — всё равно помоги.\n"
        ...
        "messages": [
            {
                ...
                "content": [
                    ...
                    },
                    {
                        ...
                        "text": (
                            "Внимательно рассмотри это фото. "
                            "Если на фото есть текст — перепиши ВЕСЬ текст дословно. "
                            "Если текста нет или мало — подробно опиши что изображено на фото. "
                            "Отвечай по-русски, без пояснений."
                        )
                    }
                ]
            }
...
def ask_ai_vision(question, system, image_base64):
    ...
    if ocr_text:
        ...
        if user_q:
            ...
            combined = f"Вот что на фото:\n\n{ocr_text}\n\nЕсли это задача — реши пошагово. Если это не задача — опиши что это и дай полезную информацию."
        ...
        return "Не удалось распознать содержимое фото. Попробуй сфотографировать чётче или опиши что на фото текстом — помогу разобраться!", 0
...
def build_smart_fallback(question, context):
    ...
    if any(w in q for w in ['задание', 'задача', 'пример', 'реши']):
        return "Готов порешать! ✏️\nНапиши тему задания конкретнее — и я дам задачу с разбором."
    return "Хороший вопрос! 🤔\nУточни подробнее — отвечу с примером."

DEMO_SYSTEM = (
    "Ты Studyfay — умный ассистент. Помогаешь с любыми вопросами: учёба, быт, техника, природа и т.д. Русский. Формулы текстом (x^2, sqrt, a/b). 1-2 эмодзи.\n"
    "Объясняй ПРОСТЫМИ СЛОВАМИ с аналогией из жизни → покажи на примере → дай полезный совет.\n"
    "Задачу — реши пошагово. Проверка ответа: реши сам → ✅ или ❌ + объясни.\n"
    "Бытовой вопрос — дай конкретный полезный ответ. Никогда не отказывай.\n"
