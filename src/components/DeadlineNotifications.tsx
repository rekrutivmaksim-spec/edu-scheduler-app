...
const DeadlineNotifications = () => {
  ...
  return (
    <Card className="p-4 sm:p-6 bg-gradient-to-br from-red-50 via-orange-50 to-yellow-50 dark:from-red-950/30 dark:via-orange-950/30 dark:to-yellow-950/30 border-2 border-red-200 dark:border-red-800 mb-6">
      <div className="flex items-center gap-3 mb-4">
        <div className="w-10 h-10 bg-gradient-to-br from-red-500 to-orange-500 rounded-xl flex items-center justify-center">
          <Icon name="Bell" size={20} className="text-white" />
        </div>
        <div>
          <h3 className="font-bold text-lg dark:text-gray-100">⏰ Ближайшие дедлайны</h3>
          <p className="text-sm text-gray-600 dark:text-gray-300">Не забудь про эти задачи</p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {notifications.map((notif) => (
          <div
            key={notif.id}
            className="flex items-start gap-3 p-3 bg-white dark:bg-gray-800 rounded-lg border-l-4 hover:shadow-md transition-shadow"
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <h4 className="font-semibold text-sm sm:text-base truncate dark:text-gray-100">{notif.title}</h4>
              {notif.subject && (
                <p className="text-xs text-gray-600 dark:text-gray-300 mt-0.5">{notif.subject}</p>
              )}
