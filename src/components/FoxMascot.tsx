...
export default function FoxMascot({ size = 120, jumping = false, className = '', blend = false }: FoxMascotProps) {
  return (
    <div
    ...
    >
      <img
        ...
        width={size}
        height={size}
        className={`w-full h-full object-cover rounded-full drop-shadow-xl ${blend ? 'opacity-95' : ''}`}
        draggable={false}
      />
    </div>
  );
