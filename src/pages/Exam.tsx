...
function getSystemPrompt(mode: string, subjectName: string, examType: string): string {
  ...
  switch (mode) {
    case 'explain':
      return `Привет! Я твой репетитор по предмету "${subjectName}" для ${label}. Спрашивай любую тему — объясню понятно и с примерами. С чего начнём?`;
    case 'practice':
      return `Давай потренируемся! Я буду давать тебе задания формата ${label} по предмету "${subjectName}" и проверять ответы. Вот первое задание:`;
    case 'weak':
      return `Давай разберём твои слабые места по предмету "${subjectName}" для ${label}. Я дам объяснение проблемной темы и задание для закрепления. Начинаем:`;
    case 'mock':
      return `Начинаем пробный ${label} по предмету "${subjectName}"! Я буду давать задания по порядку, как на настоящем экзамене. Задание №1:`;
    default:
      return '';
  }
}
