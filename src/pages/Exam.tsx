...
const buildSystemPrompt = (examType: string, subjectId: string, subjectLabel: string, mode: string): string => {
  const examLabel = examType === 'ege' ? 'ЕГЭ' : 'ОГЭ';
  const modeText = mode === 'explain'
    ? 'Объясняй тему: теория + пример задания + частые ошибки. Предложи потренироваться.'
    : 'Давай полное задание → жди ответа → разбирай ошибки. Начни сразу с задания.';
  return `Ты Studyfay — репетитор ${examLabel} по «${subjectLabel}». Отвечай только на русском, без LaTeX. Формулы текстом: a²+b²=c². Задания давай полностью со всеми вариантами. ${modeText}`;
};
